...
class TaskSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        \App\Models\Task::factory()
            ...
            // Use Eloquent to group tasks by phase_id and count them
            $taskCounts = \App\Models\Task::select('phase_id', \DB::raw('count(*) as count'))
                ->groupBy('phase_id')
                ->get();

            // Loop through the task counts and update the phases
            foreach ($taskCounts as $taskCount) {
                $phaseId = $taskCount->phase_id;
                ...
                if ($phase) {
                    // Update the task_count column for the phase
                    $phase->update(['task_count' => $count]);
                }
            }
    }
}
